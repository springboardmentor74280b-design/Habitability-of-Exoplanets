...
def main():
    ...
    X_train, X_test, freq_encoded_cols = hybrid_encode(X_train, X_test, cat_cols, threshold=10)
    ...
    X_train_scaled, X_test_scaled, scaler = standardize(X_train, X_test, cols_to_scale)
    ...
    results = {
        "Logistic Regression": (y_test, lr.predict(X_test_scaled)),
        "Linear SVM": (y_test, svm_linear.predict(X_test_scaled)),
        "SVM (RBF + PCA)": (y_test, svm_rbf.predict(X_test_final)),
        "Random Forest": (y_test, rf.predict(X_test_scaled)),
        "XGBoost": (y_test, xgb.predict(X_test_scaled))
    }
    
    plot_model_results(results, "Outputs/confusion_matrices.png")

    # Plot XGBoost Alone
    results_xgb = {"XGBoost": (y_test, xgb.predict(X_test_scaled))}
    plot_model_results(results_xgb, "Outputs/xgboost_confusion_matrix.png")
    ...
    if RUN_TSNE:
        plot_tsne(X_train_scaled, y_train)
